...
3. **Present the proposal and wait for confirmation.** Report the token implication of the chosen set as a range, and label it an estimate: sum the line counts of the confirmed modules' reference files, then add an order-of-magnitude probe cost per module on top. A number with no method behind it is a guess wearing a token count.
...
5. Dispatch each confirmed module — or a small batch of cheap, related modules — to its own subagent. Its brief: the module's `references/<module>.md` path, the discovery summary from step 1, the finding schema, and a hard cap on the reply — schema-shaped findings only, no tool transcripts, no reasoning trace. The subagent reads only its reference file, verifies against real code — project-brain first, `Read` only to confirm lines it points at — and **runs the probe each check names**; a check whose probe was not run produces no finding, not an `inferred` one. Route mechanical sweeps (literal counts, orphan-asset greps, token adoption) to a fast model tier and reasoning-heavy modules (security, reachability, architecture) to a stronger one — the same split the Refutation Mode table already tracks by model id, so record it there instead of a second place. The main thread keeps discovery and aggregation; it does not read module references or run probes itself. Where the host has no subagent mechanism, fall back to running modules sequentially in the main context and say so in the Executive Summary.
6. Collect findings using the schema above, each with its evidence tier and its ruled-out alternative. Leave `Refutation` empty; refutation (step 9) fills it.
7. **Deduplicate before refuting.** Two findings are the same finding when they share a file with overlapping line ranges, or share a symbol and the same root cause — even when two different modules reported it. Keep the copy with the higher severity, or the stronger evidence tier on a tie, and list every module that reported it. This runs before the Refutation Manifest so no refuter is ever paid to kill a duplicate.
8. **Present the Refutation Manifest and wait** — deduplicated findings collected, how many are eligible at Medium or above, how many are left raw below threshold, how many refuters that means, and the resolved mode with model ids where they are known. Answers are `all`, `critical+high only`, or `none`. A user who replied `refute all` at step 3 has pre-authorised it; print the manifest as a notice and continue. `none` stamps every finding `declined by user` — it does not silently skip the stage.
9. Refute. One subagent per eligible finding, fresh context, the brief and nothing else — on a different model than the one that produced the finding, when the host lets you choose; the Refutation Mode table already records the model ids this yields, so name it there and not twice. Escalate to a second refuter only on `undetermined` at Critical or High, telling it which probes were already inconclusive.
10. Apply the verdicts: drop `refuted` findings into the ledger, reduce `weakened` ones to the severity the refuter named, cap `undetermined` at Medium, and record the mode on every finding. Run one more light dedup pass, restricted to findings whose severity changed under refutation — step 7 saw them at their pre-refutation severity and could not have grouped them correctly.
11. Record every module's `Out of static reach` items that mattered — they become Coverage Gaps, not silent omissions.
12. **Persist the report.** Once the Output Contract is filled, write the full report to `.project-brain/audits/<YYYY-MM-DD>-audit.md` in the audited repo — and the SARIF file next to it when SARIF emission is on — then print the path. A report this size does not survive the next context compaction, and a dated file gives the next audit something to diff against. When a previous report exists in that directory, the Executive Summary names new, resolved, and unchanged findings by fingerprint, reusing the SARIF fingerprint definition even when SARIF itself is off.
...
The finished report is written to disk, not left in the transcript — see Execution Steps step 12 for the path and the diff-against-previous-run contract. This section governs its shape; that step governs its survival.
