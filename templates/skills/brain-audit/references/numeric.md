...
- [ ] Division and percentage arithmetic that discards the remainder — a discount, tax, or fee calculated as `amount * rate` with no compensating remainder handling, run against an amount where the result cannot be represented exactly (e.g. dividing by 3).
...
- [ ] Exchange-rate application: the rate's own timestamp, and whether the code applying it uses the rate valid at the transaction time versus whatever the latest cached rate happens to be. Cross-reference `temporal.md` for whether that timestamp is stored as a proper instant.
...
- [ ] Floating-point values compared for equality (`===`, `==`, `.equals()`) anywhere in the codebase — `search_code` for `===` or `==` beside a variable that is a computed float. This is a near-guaranteed false negative on values that are mathematically equal but bit-different due to accumulated rounding.
- [ ] Database numeric column precision/scale versus the application type's range — a `DECIMAL(10,2)` column paired with an application type that permits more precision, silently truncated on write with no error surfaced.
...
- [ ] `NaN` and `Infinity` reachability into a persisted or serialised value — a `NaN` written to a numeric column, or serialised into JSON (where it becomes `null` or invalid JSON depending on the serializer), silently corrupting the value with no error at the point of injection.
...
- [ ] Quantity or amount fields with no upper-bound validation before an arithmetic operation that could overflow or produce an absurd result (a quantity field multiplied by a price with no sanity check) — reachable from user input, not just internal computation.
...
| Situation | Severity |
|---|---|
| Money stored or transmitted as binary float anywhere in the chain | Critical |
