...
- [ ] Before walking every discovered flow, `Read` product.md/discovery notes (or `get_architecture`/`repo_map` for entry points where no product doc exists) to identify the flows with the highest business impact — signup, auth, checkout/payment, and the product's core write action. `trace_path` those first and fully; flows outside that set are recorded as sampled, not silently omitted, if time-boxing forces a cut (Playwright's own guidance prioritizes "the paths that make you money and the ones that page someone at night" over exhaustive coverage — Playwright/Microsoft, official framework documentation, current, https://playwright.dev/docs/best-practices).
...
| Webhook receiver | Never registered with the provider, or registered but exempted from nothing — `search_code` the CSRF/session-auth middleware registration (`protect_from_forgery`, `csrf`, a global `app.use` auth guard) and `Read` the route mount order to confirm the webhook path is explicitly exempted; Stripe warns CSRF protection "might also prevent your site from processing legitimate events," and a provider retries a rejected delivery for a bounded window then gives up silently (Stripe, official vendor documentation, current, https://docs.stripe.com/webhooks; GitHub, official product documentation, current, https://docs.github.com/en/webhooks/testing-and-troubleshooting-webhooks/troubleshooting-webhooks — a 4xx is recorded as a failed delivery within a 10-second response window; GitHub, official product documentation, current, https://docs.github.com/en/webhooks/using-webhooks/handling-failed-webhook-deliveries — failed deliveries are not auto-redelivered) | `search_code` the webhook URL/path in provider setup, deploy config, or admin notes | High |
| Feature flag referenced in code | No live declaration in flag config/provider | `search_code` a feature-flag key across every branch that checks it, then across the flag-provider's config/init call and any committed flag-definition file. A key that gates live code but is declared nowhere else is an orphaned flag, permanently pinned to whatever the SDK's default-on-missing-key behavior is, silently, rather than by anyone's decision — orphaned flags accumulate because "nobody remembers why it existed," and no flag platform removes the dead branches from the codebase itself (LaunchDarkly, official product documentation, current, https://launchdarkly.com/docs/guides/flags/technical-debt). Refuted if the flag is a documented permanent operational kill-switch, not a temporary rollout flag | Medium |
