...
- [ ] `search_code` for range-construction helpers (`startOf`, `endOf`, `setHours(0, 0, 0, 0)`, a `from`/`to` pair) that build a wall-clock start/end, then `Read` whether the construction is zone-aware — a range spanning a "spring forward" gap (a wall-clock hour that never occurs) or a "fall back" overlap (an hour that occurs twice) can silently normalise a nonexistent time to something else, or double-count the ambiguous hour.
...
- [ ] `find_callers` on the timestamp accessor/getter identified under Storage and representation above, and `Read` each call site's comparison — an instant compared to a civil datetime, or a stored UTC value compared to a freshly-constructed local `Date` without normalising both sides first.
