...
- [ ] Modules that import from many others — `find_callees` at module granularity. A module with a long callee list is coupled to all of them at once.
...
- [ ] The same business rule expressed in more than one place — a validation, a threshold, a format. `search_code` the literal value (a magic number, a regex, a status string) across the repo; more than one hit outside a shared constant is the finding.
...
- [ ] Non-obvious code with no comment explaining *why* — read the functions `repo_map` and `impact` flag as central. Comments restating *what* the code does are noise; a missing *why* on a surprising line is a real gap.
...
- Whether coupling through shared state actually causes incorrect behaviour, which depends on execution order this module cannot observe.
...
- Whether an implicit ordering requirement has ever actually been violated in production.
