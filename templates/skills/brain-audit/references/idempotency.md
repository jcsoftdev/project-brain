...
- [ ] The key is enforced by a uniqueness constraint (DB unique index, `INSERT ... ON CONFLICT`, a dedupe table with a unique key) rather than only a read-then-write existence check — `find_symbol` the write path and `Read` it to confirm which of these it uses. A "look up by key, if not found insert" without a constraint is itself a race — cross-reference `concurrency.md` for the read-modify-write pattern, but report the missing-constraint instance here since the *mechanism* is this module's concern.
...
- [ ] A replay window — `search_code` the signature-verification function for a timestamp-tolerance check: is a signed-but-stale payload (timestamp far in the past) still accepted? Stripe's default tolerance is 5 minutes, and its own docs warn against setting tolerance to 0 (that disables the check entirely). Without a window, a captured valid payload can be replayed indefinitely.
...
- [ ] Duplicate webhook deliveries from provider-side retries (the provider itself retries on a non-2xx or a timeout) versus the handler's own idempotency check — `find_symbol` the handler and `Read` its return path relative to the write: confirm it returns success only after the write is durably committed, not before, or the provider will interpret a slow success as a failure and retry a request that already landed.
...
- [ ] A poison message (one that will never succeed, e.g. malformed payload) has a dead-letter path rather than being retried indefinitely — `find_callees` from the consumer's error handler to confirm a max-attempts-then-park path exists, distinct from a message that fails only transiently. Cross-reference `scalability.md`, which owns the dead-letter-destination/drain check more broadly; report the consumer-side poison-message instance here since it is this module's retry-safety concern.
