...
- [ ] Every external call is wrapped: `search_code` for the network/filesystem/subprocess/database client calls (`fetch(`, an HTTP client, `fs.`, `spawn(`, ORM query calls) and confirm each sits inside a try/catch or has a `.catch(`.
...
- [ ] Nothing is left half-written: in the same function, check whether later steps commit independently with no rollback path if an earlier one already succeeded — that is how partial config, orphaned rows, and half-copied directories happen.
...
- [ ] Restart-safety: the process can be killed at any point and restarted without corruption. Reuse the multi-step write path read under Partial failure — a write path with no atomic commit is not restart-safe, whether or not the process actually restarts mid-write.
