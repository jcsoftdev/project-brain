...
| `vitals.md`'s per-interaction INP | A feedback interaction (hover/click/toggle) resolves visibly slower than ~200ms. Refuted if the slow-resolving interaction triggers a network request (not pure CSS motion) — the delay is latency, not an authored duration, and belongs to `performance.md` instead. The authored-duration half of this check (animation duration against the module's declared bounded-set ceiling, found via `search_code`) stays a source `read` — no bundle artefact records authored animation durations | Low |
