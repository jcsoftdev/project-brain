...
- [ ] Generated or vendored artefacts committed at any point (build output, `node_modules`, lockfile-generated files committed alongside source) — cross-reference the large-blob list above against file extensions that should be build products, not source.
...
- [ ] Stale branches — `git branch -a` (or `--no-merged`) for branches with no recent activity and no open path to being merged. Each one is either abandoned work or work nobody remembered to land.
...
- [ ] Frequency of revert commits — `git log --all --grep='^Revert'` (or the project's own revert convention). A steady trickle of reverts against one area is instability the churn ranking alone will not surface, because a revert and its original commit both count as ordinary churn.
...
| Situation | Severity |
|---|---|
| Live secret found in history, not present in HEAD | Critical |
| Secret present in HEAD and in history | Critical |
...
| Commit message convention inconsistently followed | Info / Low |
