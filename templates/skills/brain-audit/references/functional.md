...
- [ ] Partial failure leaves recoverable state — no half-written config, no orphaned rows, no lock left held. `search_code` a lock/file acquire in the same function and confirm every return path releases it. Rule out a release in a `finally`/`defer` before reporting a leak.

## Behavioural gaps

- [ ] For each core feature, list what a user would reasonably expect that is absent — missing cancel, undo, dry-run, list/inspect for something the tool creates. Anchor each to the entry-point file that would need to grow it; cross-reference `product.md`'s onboarding path.
...
- Runtime boundary-value behaviour — does the empty-string path really take the branch the code implies.
...
| Missing validation at a trust boundary | High (escalate to Security) |
