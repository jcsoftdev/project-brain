...
- [ ] The inverse gap, and the one most worth finding: a flag key read in code (`isEnabled('checkout-v2')`) that appears in no registry, no environment file, no provider config anywhere in the repo. Most flag SDKs return a hardcoded default when a key is unrecognized rather than erroring — which means a **typo'd key silently and permanently returns the default forever**, and nothing in the type system or the test suite catches it, because the call succeeds. `search_code` every literal string argument to the flag accessor and cross-check each one against the registry list built above; any string with no exact match is a candidate, but first rule out: the key is computed at runtime (`` `flag-${variant}` ``) rather than a literal, in which case check the possible expansions instead of dismissing it. This is `High` at `read`; where the mis-keyed call site gates an authorisation, paywall, rate-limit, or billing decision, `find_callers`/`trace_path` from the call site to that decision promotes it to `traced`, and `Critical`.
- [ ] The same conceptual flag under two different keys on two surfaces — a client key and a server key that were meant to name the same rollout but were typed independently at each call site. `search_code` both and compare intent, not just spelling; anchor the judgment on a concrete signal — the same commit or PR introducing both keys (cross-reference `repo-history.md` if run), matching comment text at both declarations, or both keys set together in the same environment-file block — since these don't diff cleanly the way a typo does.
- [ ] A key present in the registry that once had readers and no longer does — `find_callers` on the accessor confirms zero current readers; cross-reference `repo-history.md`'s history on the call site if run, to distinguish this from "never had a reader" above. Otherwise report both under the same no-reader finding.
...
- [ ] Distinguish "permanently on, old branch still present" (report the dead old-branch code, `find_callers` on the unreachable branch's functions) from "permanently off, new branch never shipped" (report the flag as the thing blocking a feature that otherwise looks complete) — the recommendation differs even though the static signature from the check above looks similar.

## No recorded owner or removal date

- [ ] Flags with no owner, ticket reference, or expiry recorded anywhere — `search_code` the registry, and the call site's surrounding comments, for an owner/ticket/expiry marker; not in the registry, not in a comment at the call site, not in the PR that introduced it (out of static reach to check history depth beyond what `repo-history.md` already covers). A flag with no removal criterion is a flag that will never be removed — flag removal belongs in the definition of done, the same way the feature it gates does. Report the count, not each one individually, unless a specific flag is unusually old or risky per the checks below.
- [ ] Weigh the missing owner/expiry by the type classified above: on a **Release** or **Experiment** toggle, no expiry after weeks of history — cross-reference `repo-history.md`'s age-of-flag reading if that module ran, otherwise `search_code` for a dated comment or ticket reference at the call site — is the real finding; those categories are supposed to be short-lived by design. On an **Ops** kill switch or a **Permissioning** toggle, a long lifespan with no expiry is expected, not a defect on its own; missing an *owner* is still worth flagging there, missing an *expiry* usually isn't.
...
- [ ] A flag gating only one half of a feature — e.g. the new UI is behind the flag but the backend change it depends on shipped unconditionally (or vice versa). `find_callers` on the flag accessor and `trace_path` from the UI half to the backend half it depends on — if the flag is ever flipped off after being on, the half that shipped unconditionally is now orphaned with no way back. Cross-reference `flow-integrity.md`'s half-wired-feature table — the pairing check there is the same shape, this module just adds "…and is one of the halves actually reachable only via a flag."
...
- [ ] Where the SDK call itself takes an inline default (`isEnabled('x', { default: true })`), `search_code` each accessor call site for its inline default and diff it against the value declared in the registry's own default. Two different fallback values mean the flag has two different "off" states depending on whether the call reaches the provider or falls back locally — and the local one only fires exactly when the provider is unreachable, the least-tested path.
- [ ] Percentage-rollout or user-targeting logic computed by hand in application code (a modulo on a user ID, a locally seeded random) instead of deferred to the flag provider. `search_code` for a hand-rolled bucketing function and `Read` it for whether its input is stable — hand-rolled bucketing commonly re-randomizes on each evaluation rather than being stable per user, producing a flag that flips on and off for the same person across page loads.
