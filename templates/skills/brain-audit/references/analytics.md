...
- [ ] Where the SDK is GA4 (`gtag(`, `firebase.analytics()`), `Read` each `track(`/`gtag('event', ...)` call site's event name, parameter names, and parameter count. Flag an event name or parameter name over 40 characters, a user-property name over 24 characters, or more than 25 parameters on one event — GA4 does not error on these, it drops the excess item rather than truncating it, so the defect is invisible without reading the literal. Rule out a dynamically interpolated string whose runtime value is shorter than its source form (read the interpolation before flagging on source length alone); a project not using GA4/Firebase Analytics is exempt entirely. (Google Analytics Help, "GA4 event and property limits" — https://support.google.com/analytics/answer/9267744)
...
| `network.jsonl` (query string) | The literal query string of an analytics beacon contains a value `privacy.md`'s inventory already named as personal data, independent of what the source-level `Read` of the call site showed — refuted if the value is not present in the actual outbound browser request's query string | High |
...
| `network.jsonl` (query string on the step where the sensitive URL was current) | A beacon's query string carries the full page URL with a session token or one-time link, not just a bare path — refuted if the captured URL is stripped of its query string by the SDK's own documented config before transmission | High |
