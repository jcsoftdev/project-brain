...
- [ ] No component is large enough that its responsibilities cannot be named in one sentence. `repo_map` ranks files by centrality; open the top-ranked component files and check their size and hook count — read the file to confirm the smell before reporting, since size alone is a signal, not proof.
...
- [ ] Effects declare every dependency they read, and none they do not. `search_code` for `eslint-disable-next-line react-hooks/exhaustive-deps` — each suppression is a self-admitted gap; read what the effect references against its dependency array.
...
- Real bundle size and per-chunk output — the manifest and import graph show what is *possible* to split, not what a bundler analysis would measure.
