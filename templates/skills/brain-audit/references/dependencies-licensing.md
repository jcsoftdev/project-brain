...
- [ ] Dependencies that duplicate the standard library or the runtime. Cross-check the import list against the language's builtins — a padding or deep-clone library where the runtime already ships one is the concrete case.
...
- [ ] Lockfile present and committed. Confirm via the file listing or `get_architecture`, then check `.gitignore` does not exclude it. Absent lockfile means no two installs are the same build.
...
- [ ] Something updates dependencies on a cadence. This is the same Renovate/Dependabot check made under Health — cross-reference it rather than re-checking.

## Licensing

- [ ] Every dependency's license is known, including transitive ones. Read the manifest/lockfile license fields, or a generated license report if one exists in-repo — a checked-in SBOM (CycloneDX ≥1.4 or SPDX ≥2.3 both carry a license field per component) counts as this report — and flag any `UNKNOWN` or missing entry. Unknown is not the same as permissive.
- [ ] No license incompatible with how this project ships. For each declared license, name the specific obligation it carries — GPL requires derivative-works disclosure, for instance — checked against the project's own declared license and its shipping model (proprietary vs. open, distributed vs. SaaS). Flag the obligation; do not give a legal verdict.
...
- Real bundle-size impact of a heavy dependency — needs a build to measure.
