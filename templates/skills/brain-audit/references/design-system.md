...
- [ ] Only when the token source or its build tooling (a Style Dictionary config, a Tokens Studio/Figma-plugin export, a file named `tokens.json` with `$value`/`$type` keys) declares or implies DTCG-format compliance, `Read` the file and check every token carries `$value` and that its type is *determinable* — declared via its own `$type`, inherited from the closest parent group's `$type`, or resolved through an alias to a token whose type is determinable — that every alias uses `{group.token}` syntax rather than a duplicated raw literal, and that no group name begins with `$` or contains `{`, `}`, `.`. A project that never claimed this format (a plain Tailwind config, a styled-components theme object, a CSS custom-property sheet) is out of scope for this check entirely (Design Tokens Community Group, W3C Community Group, 2026, https://www.designtokens.org/TR/drafts/format/ — a token requires `$value`; `$type` is optional on a token and MUST be determined from the referenced token's resolved type or inherited from the closest parent group when omitted; an alias resolves via `{group.token}` syntax).
