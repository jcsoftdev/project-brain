...
- [ ] Action, image, and tool versions are pinned — owned by `supply-chain.md` (`search_code` workflow files for `uses: <action>@v` without a SHA, and the Dockerfile `FROM` tag vs. digest); reuse its finding, do not re-report.
