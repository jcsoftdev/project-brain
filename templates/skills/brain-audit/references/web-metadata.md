...
- [ ] No structured data type the project emits is one Google has stopped consuming. `search_code` the `@type` values inside each `application/ld+json` block and check against the current retirement list — `FAQPage` (rich-result support removed May 2026), and `Book Actions`, `Course Info`, `ClaimReview`, `Estimated Salary`, `LearningVideo`, `SpecialAnnouncement`, and `VehicleListing` (removed June 2025). A hit is not broken code — it still validates and runs — but it is dead investment: nothing on the results page consumes its output any more.
...
| Situation | Severity |
|---|---|
| `noindex` shipped to production on an indexable route | Critical |
