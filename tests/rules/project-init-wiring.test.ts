...
describe("runInit project rules wiring (T-6.2)", () => {
  ...
  it("runInit accepts skipRules option without error", async () => {
    ...
    await expect(
      runInit({ root: tempDir, dataDir: join(tempDir, ".project-brain"), skipGitHook: true, skipRules: true, skipIndex: true })
  ...
  it("runInit does NOT write CLAUDE.md when skipRules is true", async () => {
    ...
    await runInit({ root: tempDir, dataDir: join(tempDir, ".project-brain"), skipGitHook: true, skipRules: true, skipIndex: true });
